...
void ZigGenerator::generateGlobalVars() {
    ...
    globalVars.push_back("var prng = std.rand.DefaultPrng.init(0);");
    globalVars.push_back("const random = prng.random();");
}
...
void ZigGenerator::generateRandomNumberGenerator() {
    ...
    rngFunction.addLine({
        "pub fn get_path() !u64 {",
        "    if (try std.process.getEnv(allocator, \"BENCH_PATH\")) |path| {",
...
void ZigGenerator::generateMainFunction() {
    ...
    mainFunction.addLine({
        ...
        "    prng = std.rand.DefaultPrng.init(path_seed);",
        "}"
    });
...
void ZigGenerator::generateFiles(std::string benchmarkName) {
    ...
    mainFile << "var gpa = lib.gpa;\n";
    mainFile << "var prng = lib.prng;\n";
    mainFile << "const random = lib.random;\n";
    mainFile << "const " + VariableFactory::genTypeString(varType) + " = lib." + VariableFactory::genTypeString(varType) + ";\n";
    mainFile << "const " + VariableFactory::genTypeString(varType) + "Param = lib." + VariableFactory::genTypeString(varType) + "Param;\n";
