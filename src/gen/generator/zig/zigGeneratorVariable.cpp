...
std::vector<std::string> ZigGeneratorArray::genIncludes() { return {}; }
...
std::vector<std::string> ZigGeneratorArray::genGlobalVars() {
  ...
  temp.push_back("");
  temp.push_back("pub const " + this->typeString + " = struct {");
  ...
  temp.push_back("};");
  temp.push_back("");
  temp.push_back("pub const " + this->typeString + "Param = struct {");
...
ZigGeneratorArray::genParams(std::string paramName,
  ...
  temp.push_back("const " + dataSliceName + " = try allocator.alloc(*" +
